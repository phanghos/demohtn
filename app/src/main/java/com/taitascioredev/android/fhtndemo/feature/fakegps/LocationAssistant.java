...
public class LocationAssistant
    ...
    private void checkMockLocations() {
        ...
        if (Build.VERSION.SDK_INT < 18 &&
            ...
            if (listener != null)
                listener.onNewLocationAvailable(new SafeLocation(null, true));
        } else
    ...
    public void onLocationChanged(Location location) {
        ...
        if (!allowMockLocations && !plausible) {
            if (listener != null) listener.onNewLocationAvailable(new SafeLocation(location, true));
            return;
        }

        bestLocation = location;
        if (listener != null)
            listener.onNewLocationAvailable(new SafeLocation(location, false));
        else if (!quiet)
